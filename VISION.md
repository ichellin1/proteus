...
### V1: Textured Rectangles with Instanced Rendering
...
All components are rendered in a **single instanced draw call**. Per-instance data — transform, UV mapping, color, opacity — is packed into one homogeneous GPU buffer. The entire scene is submitted to the GPU with one buffer update and one draw call per frame, regardless of how many components are present.
...
- **Minimal WASM boundary crossings.** On the web, the WASM→JS boundary is crossed once per frame to update the instance buffer and issue the draw call — O(1) regardless of component count. This is the recommended rendering strategy for web targets.
- **Predictable performance.** Rendering cost scales with component count on the GPU only, where it is cheap. The CPU-side cost per frame is constant.

### Future Geometry Types

Introducing geometry types beyond quads breaks the single homogeneous buffer model. Two strategies are under consideration for future iterations:

**Option 1 — Multiple instance buffers (one per geometry type).** One instanced draw call per type. O(1) boundary crossings per type, O(types) draw calls total. Simple to implement and well-suited to native targets where draw call overhead is lower.

**Option 2 — Geometry atlasing.** All geometry types are expressed as meshes and packed into a shared vertex and index buffer. More complex to manage but maintains a unified pipeline. Better suited to native targets handling large numbers of diverse geometry types.

The decision between these two approaches will be made during a future iteration, informed by benchmarks on native targets. The V1 single-buffer model is not replaced by either — it remains the recommended approach for web.
