...
/// `images/{tiger,sintel_fixed,jellyfish_fixed}.mp4` — see
/// `logo_frame_path`'s doc for why this points at
/// `proteus-shell-native`'s own asset directory. Index order matches
/// `screens::video_tiles`' left/center/right tiles (same order as
/// `TILE_IMAGE_PATHS`).
const TILE_VIDEO_PATHS: [&str; 3] = [
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../proteus-shell-native/assets/videos/tiger.mp4"
    ),
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../proteus-shell-native/assets/videos/sintel_fixed.mp4"
    ),
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../proteus-shell-native/assets/videos/jellyfish_fixed.mp4"
    ),
];
...
/// treatment set `Demo::set_loading_logo_frames_dark` wants. See
/// `logo_frame_path`'s doc for why this points at
/// `proteus-shell-native`'s own asset directory.
...
const BG_IMAGE_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/bg/ocean-blur-dark.jpg"
);

/// `images/icons/*.png` and `images/logo/lockup*.png` — see
/// `logo_frame_path`'s doc for why these point at
/// `proteus-shell-native`'s own asset directory. Order matches
/// `Demo::set_nav_*`'s 8 setters.
const NAV_HOME_ICON_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/home-idle.png"
);
const NAV_HOME_ICON_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/home-idle-dark.png"
);
const NAV_HOME_ICON_SELECTED_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/home-selected.png"
);
const NAV_HOME_ICON_SELECTED_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/home-selected-dark.png"
);
const NAV_BACK_ICON_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/back-idle.png"
);
const NAV_BACK_ICON_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/back-idle-dark.png"
);
const NAV_LOGO_LOCKUP_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/logo/lockup.png"
);
const NAV_LOGO_LOCKUP_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/logo/lockup-dark.png"
);
...
/// pair here does — see `screens::theme`'s own module doc for the inverted
/// asset-role convention `sun`/`sun_dark` use.
const THEME_SUN_ICON_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/sun-idle-dark.png"
);
const THEME_SUN_ICON_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/sun-selected.png"
);
const THEME_MOON_ICON_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/moon-idle.png"
);
const THEME_MOON_ICON_DARK_PATH: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../proteus-shell-native/images/icons/moon-selected-dark.png"
);

/// `images/{tiger,sintel,jellyfish}.jpg` — see `logo_frame_path`'s doc for
/// why this points at `proteus-shell-native`'s own asset directory. Index
/// order matches `screens::video_tiles`' left/center/right tiles.
const TILE_IMAGE_PATHS: [&str; 3] = [
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../proteus-shell-native/images/tiger.jpg"
    ),
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../proteus-shell-native/images/sintel.jpg"
    ),
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../proteus-shell-native/images/jellyfish.jpg"
    ),
];
...
/// Mirrors `proteus-shell-native`'s own identical pre-bake loop (run twice
/// there too, once per set). Missing/unreadable frames degrade gracefully
/// — same convention as any other image asset.
fn bake_logo_frames(
    app: &mut Proteus,
    queue: &wgpu::Queue,
...
/// specific entity). Mirrors
/// `proteus-shell-native::bake_pending_images`/`bake_gallery_hires_image`'s
/// shared decode→resize→register→upload→insert shape, minus
/// `bake_pending_images`' own gallery-specific center-crop handling (a
/// separate step in this crate — see `Demo::advance_gallery_tile_crop`'s
/// doc).
fn bake_images(
    world: &mut bevy_ecs::world::World,
    queue: &wgpu::Queue,
...
/// time, so it can be bigger (900px, matching
/// `proteus-shell-native::GALLERY_LARGE_IMAGE_MAX_SIDE` exactly). Must run
/// before `bake_pending_images` each frame — see that function's doc.
...
/// can't do (it's headless). Skips a cycle (matches
/// `proteus-shell-native::churn_texture`'s own graceful degradation) rather
/// than erroring if the atlas is full even after eviction.
...
/// half of video playback `Demo` itself can't do (it's headless). Mirrors
/// `proteus-shell-native::start_video_playback`/`stop_video_playback`,
/// using the same `mp4_player` module (copied verbatim — see that module's
/// doc) and `QuadPipeline::init_video`/`suspend_video`.
...
/// `Demo` itself can't do (it's headless). Mirrors
/// `proteus-shell-native::start_home_to_loading`/`drain_gallery_fetch`,
/// using the same `gallery_fetch` module (adapted — see that module's
/// doc).
///
...
/// here, mirroring `proteus-shell-native::start_home_to_loading`/
/// `start_gallery_to_loading`'s identical `gallery_cell_size(..) *
/// scale_factor, capped at MAX_TILE_IMAGE_SIDE` — without this, tiles
/// fetch at half their intended resolution (or worse) on any HiDPI
/// display.
fn apply_gallery_fetch(
    ...
    /// `proteus-shell-native::MAX_TILE_IMAGE_SIDE`.
...
/// enlarged view `Demo` itself can't do. Mirrors `proteus-shell-
/// native::start_gallery_to_image`/`drain_gallery_hires_fetch`/
/// `cancel_gallery_hires_fetch`.
///
...
/// `GALLERY_LARGE_IMAGE_MAX_SIDE_PX` here, mirroring `proteus-shell-
/// native::start_gallery_to_image`'s identical `to.size * scale_factor,
/// capped at GALLERY_LARGE_IMAGE_MAX_SIDE`.
fn apply_gallery_hires_fetch(
    ...
    /// `proteus-shell-native::GALLERY_LARGE_IMAGE_MAX_SIDE`.
...
impl State {
    async fn new(window: Arc<Window>) -> Self {
        ...
        // transparent (which, without this, is exactly what every theme
        // icon looked like — reported directly as "white circles").
    ...
    fn render(&mut self) {
        // Clamped to 20fps-equivalent — `proteus-shell-native::render` does
        // the same (`.min(0.05)`). Without this, any real stall between
        ...
        // delay+fade+hold budget (`splash::INTRO_DELAY_SECS` +
        // `INTRO_DURATION_SECS` + `HOLD_SECS`, ~3.1s) in a single tick —
        // the intro/logo animation never gets a chance to render a single
        // visible frame, and the app appears to open straight into the
        // Splash→Home transition already in flight.
