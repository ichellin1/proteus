...
//!
//! Copied verbatim from `proteus-shell-native/src/mp4_player.rs` — fully
//! self-contained (only depends on `proteus_render::VideoFrameSender`, no
//! window/GPU-handle references), so it drops directly into this preview
//! harness unchanged. Once M12.5's cutover (Step 9) actually wires
