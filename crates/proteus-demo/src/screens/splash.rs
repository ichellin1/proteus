...
/// How long Splash waits, fully invisible, before the intro fade/slide-in
/// starts.
pub const INTRO_DELAY_SECS: f32 = 1.0;
/// Seconds for the entry fade (opacity 0 → 1, in lockstep with the
/// slide-in).
pub const INTRO_DURATION_SECS: f32 = 0.6;
/// How far left of its resting (centered) position the composite starts,
/// sliding in as it fades.
pub const INTRO_SLIDE_DISTANCE_PX: f32 = 250.0;

/// How long Splash holds, once the intro fade/slide-in has fully settled,
...
/// The mark's native aspect ratio is 104:144 (13:18, portrait), which is
/// what `LOGO_MARK_WIDTH` derives from below.
const LOGO_MARK_HEIGHT: f32 = 200.0;
const LOGO_MARK_WIDTH: f32 = LOGO_MARK_HEIGHT * 104.0 / 144.0;

/// The mark+wordmark composite's resting size, applied as `button`'s own
/// `QuadState::scale` so it scales
...
/// The wordmark bakes at full resolution and is only *displayed* smaller
/// (via `COMPOSITE_SCALE`),
...
/// yield the same result. `slide_offset` is `Demo::advance_intro`'s
/// contribution (0 once the intro has fully settled).
